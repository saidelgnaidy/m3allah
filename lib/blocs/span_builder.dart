...
class SpanEntity {
  const SpanEntity({required this.start, required this.end, required this.span})
      : assert(start >= 0),

        /// we only accept _ManagedTextSpans since we must take care of managing the recognizer
        assert((span is TextSpan && span is _ManagedTextSpan) || span is! TextSpan),
        assert(end > start);
  ...
  bool overlaps(SpanEntity other) {
    return (other.start < end && other.start >= start) || (other.end <= end && other.end >= start);
  }
...
extension ManagedTextSpanExtension on TextSpan {
  InlineSpan managed(RecognizerBuilder recognizerBuilder) => _ManagedTextSpan.fromTextSpan(this, recognizerBuilder: recognizerBuilder);
}
...
class _ManagedTextSpan extends TextSpan {
  const _ManagedTextSpan({
    required this.recognizerBuilder,
    String? text,
    TextStyle? style,
    String? semanticsLabel,
  }) : super(text: text, style: style, semanticsLabel: semanticsLabel);
  _ManagedTextSpan.fromTextSpan(TextSpan textSpan, {required RecognizerBuilder recognizerBuilder})
      : this(text: textSpan.text, style: textSpan.style, semanticsLabel: textSpan.semanticsLabel, recognizerBuilder: recognizerBuilder);
  final RecognizerBuilder recognizerBuilder;

  TextSpan withManagedRecognizer(GestureRecognizer managedRecognizer) {
    return TextSpan(style: style, text: text, semanticsLabel: semanticsLabel, recognizer: managedRecognizer);
  }
}
...
class SpanBuilder {
  ...
  SpanBuilder apply(InlineSpan span, {String? whereText, int? from, int? to, Function()? onTap, RecognizerBuilder? recognizerBuilder}) {
  ...
  SpanBuilder add(SpanEntity newEntity) {
    ...
    for (final entity in _entities) {
      if (entity.overlaps(newEntity)) {
        throw StateError("Unable to add $newEntity. Overlaps with existing $entity");
      }
  ...
  List<InlineSpan> build({_RecognizerBuilder? recognizerBuilder}) => _computeSpans(sourceText, _entities, recognizerBuilder!);
}
...
class SpanBuilderWidget extends StatefulWidget {
  ...
  State<StatefulWidget> createState() => _SpanBuilderWidgetState(textSpan: const TextSpan(children: []));
}
...
class _SpanBuilderWidgetState extends State<SpanBuilderWidget> {
 TextSpan textSpan;
  ...
  _SpanBuilderWidgetState({required this.textSpan});
  ...
  void _instigate() {
    ...
    textSpan = TextSpan(children: widget.text.build(recognizerBuilder: recognizerBuilder), style: widget.defaultStyle);
  }
...
List<InlineSpan> _computeSpans(String text, List<SpanEntity> entities, _RecognizerBuilder recognizerBuilder) {
